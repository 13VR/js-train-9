...
function findIndexByProperty(arr, name) {
  ...
  const findName = arr.findIndex((element) => element.name === name);
  if (findName !== -1) {
    return findName;
  } else {
    return -1;
...
function findLastEvenNumber(arr) {
  ...
  const lastNumber = arr.findLast((element) => element % 2 === 0);
  if (lastNumber % 2 === 0) {
    return lastNumber;
  } else {
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  const slice = arr.slice(startIdx).join(" ");
  return slice;
}
...
function getPositiveArrayValues(arr) {
  ...
  const iterator = Array.from(arr.values());
  const result = iterator.filter((element) => element > 0);
  return result;
}
...
function removeAndDouble(arr, startIdx) {
  ...
  const elm1 = arr.splice(startIdx);
  // const result = elm1.map((x) => x * 2);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const initialValue = 0;
  let elem1 = arr.slice(n + 1);
  const elem2 = elem1.reduceRight(
    (accumulator, currentValue) => accumulator + currentValue,
    initialValue
  );
  elem1.forEach((element) => console.log(element));
  return elem2;
}
...
function expandArrayByN(arr, n) {
  ...
  const newArray = arr.flatMap((num) => {
...
function findDuplicateElements(arr) {
  ...
  const filter = arr.filter((number, index) => arr.indexOf(number) !== index);
  return filter;
}
...
function calculateTotalPrice(arr) {
  ...
  const sum = arr.reduce((accumulator, currentValue) => {
    if (currentValue && typeof currentValue.price === "number") {
...
function findLastNegativeNumberIndex(arr) {
  ...
  const last = arr.findLastIndex((element) => element < 0);
  return last;
}
